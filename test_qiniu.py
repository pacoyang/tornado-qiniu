...
from tornaqiniu import Auth, set_default, etag, PersistentFop, build_op, op_save
from tornaqiniu import put_data, put_file, put_stream
from tornaqiniu import BucketManager, build_batch_copy, build_batch_rename, build_batch_move, build_batch_stat, build_batch_delete
from tornaqiniu import urlsafe_base64_encode, urlsafe_base64_decode

from tornaqiniu.compat import is_py2, b

from tornaqiniu.services.storage.uploader import _form_put

import tornaqiniu.config
...
class BucketTestCase(AsyncTestCase):
    ...
    @async_test
    def test_list(self):
        ret,eof,info = yield tornado.gen.Task(self.bucket.list,bucket_name, limit=4)
        print(ret.get('items'))
        assert eof is False
        assert len(ret.get('items')) == 4
        self.stop()

        # def limit100_back(ret,eof,info):
        #     print(info)
        #     assert eof is True
        #     assert len(ret.get('items')) == 100
        #     self.stop()
        # self.bucket.list(bucket_name, limit=100,callback=limit100_back)
        # self.write()

    # def test_buckets(self):
    #     def buckets_back(ret,info):
    #         print(info)
    #         assert bucket_name in ret
    #         self.stop()
    #     self.bucket.buckets(callback=buckets_back)
    #     self.wait()
